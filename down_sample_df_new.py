...
def find_anomalies_mean_L1(filename):
    ...
    for class_label in class_labels:
        ...
        ax = fig.add_subplot(len(class_labels), 1, 1)   # count change count to using class_label: assumes classes are ordered 1-n though
        ax.set_ylim(df_min, df_max)
        plot_period_std(ax, class_mean, class_std)
        #plot_outliers(ax, dfCOutliers.iloc[0, :])
        dfCOutliers.apply(lambda x: plot_outliers(ax, x, 'g', 1, 2), axis=1)
        #dfCNonOutliers.apply(lambda x: plot_outliers(ax, x, 'b', 0.1), axis=1)
        ax.set_title("RR_lyrae outliers")
        if ax_count < len(class_labels):
            plt.xticks([])   # overlaps with titles
        ax_count += 1

        ax = fig.add_subplot(len(class_labels), 1, 2)   # count change count to using class_label: assumes classes are ordered 1-n though
        ax.set_ylim(df_min, df_max)
        plot_period_std(ax, class_mean, class_std)
        #plot_outliers(ax, dfCOutliers.iloc[0, :])
        dfCOutliers.apply(lambda x: plot_outliers(ax, x, 'g', 1, 3), axis=1)
        #dfCNonOutliers.apply(lambda x: plot_outliers(ax, x, 'b', 0.1), axis=1)
        ax.set_title("RR_lyrae outliers phase shifted")
        if ax_count < len(class_labels):
            plt.xticks([])   # overlaps with titles
        ax_count += 1
